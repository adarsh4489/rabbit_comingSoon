import dark from "../assets/headerImg/night.svg"
import light from "../assets/headerImg/Group 2.svg"
import { FaInstagramSquare,FaFacebook,FaGithub,FaLinkedin,FaYoutube,FaPinterest,FaUser } from "react-icons/fa";

const Menu = ({lightMode,setLightMode}) => {
  return (
    
        <div className='w-[80%] md:w-[40%] lg:w-[18%] z-50 menu-container flex flex-col gap-[2px] items-center justify-center text-center '>
          <div className="text-lg font-semibold">Paper Rabbit</div>
            <div className='w-full bg-white rounded-t-3xl py-1'><a href="/">Home</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Hutch</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Services</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Portfolio</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Lounge</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">About Us</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Careers</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Store</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">World</a></div>
            <div className='bg-white w-full py-1 hover:bg-gray-200'><a href="">Contact Us </a></div>
            <div className='bg-white w-full py-1 flex justify-center gap-10 hover:bg-gray-200 relative'><a href="">Client Login</a>
            <span className="absolute right-5 top-2 cursor-pointer hover:scale-110"><FaUser/></span>
            </div>
            <div className='rounded-b-3xl w-full py-3 hover:bg-gray-200 bg-white flex   text-xl justify-between px-6 items-center'>
            <span className="cursor-pointer hover:scale-110"><FaInstagramSquare/></span>
    <span className="cursor-pointer hover:scale-110"><FaFacebook/></span>
    <span className="cursor-pointer hover:scale-110"><FaGithub/></span>
    <span className="cursor-pointer hover:scale-110"><FaLinkedin/></span>
    <span className="cursor-pointer hover:scale-110"><FaPinterest/></span>
    <span className="cursor-pointer hover:scale-110"><FaYoutube/></span>
            </div>
            <div className="flex items-center justify-center pt-2">
            {lightMode ? (
              <img
                className="w-10 h-full"
                src={dark}
                alt="darkmode"
                onClick={()=>{setLightMode(!lightMode)}}
              />
            ) : (
              <img
                className="w-10 h-full"
                src={light}
                alt="darkmode"
                onClick={()=>{setLightMode(!lightMode)}}
              />
            )}

          </div>

    </div>
  )
}

export default Menu